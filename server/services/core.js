...
const getLanguageLinks = async (config, page, contentType, defaultURL) => {
  ...
  await Promise.all(
    page.localizations.map(async (translation) => {
      ...
      const translationUrl = await strapi.plugins.sitemap.services.pattern.resolvePattern(
        pattern,
        translation
      );
  ...
  await Promise.all(
    Object.keys(config.contentTypes).map(async (contentType) => {
      ...
      for (let i = 0; i <= Object.keys(current).length; i++) {
        const exists =
          current[`condition${i}`] &&
          current[`conditionOperator${i}`] &&
          current[`conditionValue${i}`];
        if (exists) {
...
const createSitemap = async (cache, invalidationObject) => {
  ...
  try {
    const { sitemapEntries, cacheEntries } = await createSitemapEntries(invalidationObject);
    ...
    if (isEmpty(allEntries)) {
      strapi.log.info(
        logMessage('No sitemap XML was generated because there were 0 URLs configured.')
      );
      return;
    }
    ...
    strapi.log.error(
      logMessage(`Something went wrong while trying to build the SitemapStream. ${err}`)
    );
    throw new Error();
  }
};
